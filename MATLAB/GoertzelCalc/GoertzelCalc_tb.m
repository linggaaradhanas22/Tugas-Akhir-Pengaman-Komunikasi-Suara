...
% Plot the input DTMF signal
figure('Name', 'Input DTMF Signal', 'Position', [100, 100, 1200, 400]); % Makes figure window wider
plot(t, xDTMF, 'LineWidth', 1.5); % Increased line width for better visibility
title('Input DTMF Signal Digit 9 (852 Hz + 1477 Hz)');
xlabel('Time (s)');
ylabel('Amplitude');
xlim([0 0.02]); % Sets x-axis from 0 to 0.02 seconds
ylim([-2 2]); % Keeps current y-axis limits
grid on;
set(gca, 'FontSize', 12); % Increases font size for better readability
